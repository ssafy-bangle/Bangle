...
public class OpenAIService {
	...
	public CommentResponse makePrompt(CommentRequest commentRequest) {
		...
		// prompt.append(commentRequest.getGenre())
		// 	.append("너는 이미지 생성을 돕는 프롬프트를 작성할 꺼야. 이 프롬프트는 dall-e 2 모델에 적용될거야")
		// 	.append("책 제목은 ").append(commentRequest.getTitle()).append("이고, ")
		// 	.append("장르는 ").append(commentRequest.getGenre()).append("야. ")
		// 	.append("줄거리는 ").append(commentRequest.getComment()).append("이야. ")
		// 	.append("이 줄거리를 바탕으로 그림으로 그릴 수 있게 특징을 자세하게 추출해서 프롬프트를 작성해줘. ")
		// 	.append("문장의 시작은 'a illustration of'로 시작해. ")
		// 	.append("프롬프트를 작성할 때 주의할 점은 두 가지가 있어."
		// 		+ " 프롬프트 길이가 700자 보다 작아야하고, dall-e 2 모델에 부합하는 내용도 없어야해 ");
		prompt.append("너는 이미지 생성을 돕는 프롬프트를 작성할 꺼야. 이프롬프트는 dall-e 2 모델에 적용될거야. ")
